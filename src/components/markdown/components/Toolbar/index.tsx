...
export function ToolbarItems(props: IToolbarProps) {
  ...
  return (
    <ul className='flex flex-wrap w-max gap-3 px-3 border-b-2 list-none'>
      {(props.commands || []).map((item, idx) => {
        if (item.keyCommand === 'divider') {
          return <li key={idx} {...item.liProps} className={`h-4 w-[1px] mt-1 mx-1 bg-black/20 ${prefixCls}-toolbar-divider`} />;
        }
        ...
        return (
          <li key={idx} {...item.liProps} className={cn(activeBtn ? `active` : '', '')}>
...
export default function Toolbar(props: IToolbarProps = {}) {
  ...
  return (
      <div className={`flex flex-row justify-between flex-wrap h-14 py-3 ${prefixCls}-toolbar ${className}`}>
        <ToolbarItems {...props} commands={props.commands || commands || []}/>
        <div className='flex -mt-2.5 flex-grow w-max border-b-2'>
